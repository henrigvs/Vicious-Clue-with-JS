...
from src.backend.riddles.domain.RiddleModel import RiddleModel
from src.backend.riddles.domain.clues.Clue import Clue
from src.backend.riddles.domain.clues.ClueModel import ClueModel
...
class RiddleRepository:
    ...
    def getAllRiddlesOfAnOwner(ownerId: str) -> List[Riddle]:
        riddles = db.session.query(RiddleModel).filter(RiddleModel.fk_ownerId == ownerId).all()
        if riddles:
            for riddle in riddles:
                riddle.clues = db.session.query(ClueModel).filter(ClueModel.fk_riddle_id == riddle.riddle_id).all()
        return [riddle.to_realRiddleObject() for riddle in riddles] if riddles else []
    ...
    @staticmethod
    def increaseDifficulty(riddleId: str) -> Optional[Riddle]:
        riddleDifficulty = RiddleRepository.getRiddleByID(riddleId).difficulty
        riddle = None
        # Validate difficulty
        if riddleDifficulty < 5:
            db.session.query(RiddleModel).filter(RiddleModel.riddle_id == riddleId).update(
                {
                    RiddleModel.difficulty: riddleDifficulty + 1,
                }
            )
    ...
    @staticmethod
    def decreaseDifficulty(riddleId: str) -> Optional[Riddle]:
        riddleDifficulty = RiddleRepository.getRiddleByID(riddleId).difficulty
        riddle = None
        # Validate difficulty
        if riddleDifficulty > 0:
            db.session.query(RiddleModel).filter(RiddleModel.riddle_id == riddleId).update(
                {
                    RiddleModel.difficulty: riddleDifficulty - 1,
                }
